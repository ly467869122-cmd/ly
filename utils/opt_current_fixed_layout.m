﻿function results = opt_current_fixed_layout(spec_in)
% Optimize current for one fixed candidate or regular-grid layout.
%
% Candidate current scan:
%   spec = struct();
%   spec.source.results_mat = 'D:\...\results_data.mat';
%   spec.target.layout_mode = 'candidate';
%   spec.target.candidate_id = 23;
%   spec.current_opt.I_list = 2:0.1:4;
%   results_I = opt_current_fixed_layout(spec);
%
% Regular-grid current scan:
%   spec = struct();
...
%   results_I = opt_current_fixed_layout(spec);
%
% Single-current temperature recalculation:
%   spec.current_opt.I_list = 3.8;
%   results_T = opt_current_fixed_layout(spec);
%
% Required input for candidate mode:
%   spec.source.results_mat       path to results_data.mat from layout search
%   spec.target.candidate_id      candidate_id to scan
% Optional input:
%   spec.target.layout_mode       'candidate' (default) or 'regular_grid'
%   spec.current_opt.I_list       default 1:0.2:6
%   spec.current_opt.qc_abs_tol   default 1e-3 W
%   spec.current_opt.qc_rel_tol   default 1e-3
...
spec = apply_current_opt_defaults_ms(spec_in);
mkdir_if_needed_ms(spec.output.output_dir);

fprintf('\n========== opt_current_fixed_layout ==========' );
...
base_spec = merge_current_opt_runtime_spec_ms(base_spec, spec);
...
base_spec = ensure_current_opt_parallel_pool_ms(base_spec);
spec.use_parallel = base_spec.use_parallel;
spec.parallel = base_spec.parallel;
fprintf('Parallel: requested=%d, used=%d, workers=%d\n', ...
    parallel_requested, logical(base_spec.use_parallel), get_parallel_worker_count_ms());

scan = run_current_scan_for_candidate_ms(cand, G, base_spec, spec);
write_current_opt_outputs_ms(scan, cand, base_spec, spec, source_name);
...
results.best_I = scan.best_I;
results.best_DeltaTN_actual = scan.best_DeltaTN_actual;
results.has_feasible_solution = scan.has_feasible_solution;
results.output_dir = spec.output.output_dir;
fprintf('Done. has_feasible_solution=%d, best_I=%.6g A, best_DeltaTN=%.6g K\n', ...
    scan.has_feasible_solution, scan.best_I, scan.best_DeltaTN_actual);
fprintf('============================================================\n\n');
end
...
if ~isfield(spec.output, 'output_dir') || isempty(spec.output.output_dir)
    if is_regular_grid_mode_ms(spec)
        base_dir = fullfile(pwd, 'current_opt_regular_grid');
    ...
    run_name = sprintf('current_opt_%s_I%s-%s_n%d_%s', ...
        target_tag, i_min_tag, i_max_tag, i_count, stamp);
...
base_spec.current_opt = spec.current_opt;
...
function spec = ensure_current_opt_parallel_pool_ms(spec)
if ~isfield(spec, 'parallel') || ~isstruct(spec.parallel)
...
if ~has_parallel
    warning('Current optimization parallel requested but Parallel Computing Toolbox functions are unavailable; falling back to serial.');
    spec.use_parallel = false;
    return;
end
...
try
    ...
    elseif pool.NumWorkers ~= worker_target
        fprintf('[CurrentScan] existing parallel pool workers=%d, requested=%d; using existing pool.\n', ...
    ...
    warning('Current optimization parallel pool unavailable: %s. Falling back to serial.', ME_pool.message);
    spec.use_parallel = false;
end
end
...
use_par = logical(base_spec.use_parallel) && N >= 2 && get_parallel_worker_count_ms() > 0;
if use_par
    fprintf('[CurrentScan] evaluating %d current points in parallel\n', N);
    parfor i = 1:N
        [records(i), rows(i)] = evaluate_current_scan_one_ms(i, I_list(i), cand, G, base_spec, qc_tol);
    end
    for i = 1:N
        fprintf('[CurrentScan] I=%.6g success=%d target_met=%d DeltaTN=%.6g Qc_last=%.6g msg=%s\n', ...
            rows(i).I_A, rows(i).success, rows(i).target_met, rows(i).DeltaTN_actual, rows(i).Qc_last_total, rows(i).message);
    end
else
    for i = 1:N
        Ii = I_list(i);
        fprintf('[CurrentScan] %d/%d: I=%.6g A ...\n', i, N, Ii);
        [records(i), rows(i)] = evaluate_current_scan_one_ms(i, Ii, cand, G, base_spec, qc_tol);
        fprintf('[CurrentScan] I=%.6g success=%d target_met=%d DeltaTN=%.6g Qc_last=%.6g msg=%s\n', ...
...
if isfield(scan, 'parallel_used')
    fprintf(fid, 'parallel_used=%d\n', logical(scan.parallel_used));
end
if isfield(scan, 'parallel_workers')
...
function [rec, row] = evaluate_current_scan_one_ms(scan_idx, Ii, cand, G, base_spec, qc_tol)
cand_i = cand;
cand_i.I_opt = Ii;
G_i = G;
G_i.I = Ii;
rec = evaluate_single_candidate_ms(cand_i, G_i, base_spec, true);
row = build_current_scan_row_ms(scan_idx, rec, base_spec, qc_tol);
end
